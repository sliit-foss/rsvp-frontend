...
const LoginForm = ({login,loginToggleHandler}:LoginFormProps): JSX.Element => {
    return(
        <div className="flex flex-col order-1 lg:order-none items-center lg:w-6/12 lg:justify-center lg:transform lg:scale-90">
            ...
            <p className="text-sm border-b-2 w-full lg:w-4/5 text-center mb-2" style={{color:'#D2D2D2',lineHeight:"0.1rem", borderColor:'#D2D2D2'}}><span className="px-3 bg-white">{login ? 'or Sign in with Email' : 'or Signup with Email'}</span></p>
            {login ? loginFormFields : signupFormFields}
            <button className="w-full lg:w-4/5 bg-lightBlue rounded-lg font-inter text-sm leading-6 text-white font-semibold h-10 mt-4 mb-2">{login ? 'Login' : 'Sign Up'}</button>
            <div className="w-full lg:w-4/5">
                <p className="text-xs text-left font-semibold">{login ? 'Not Registered Yet?': 'Already have an account?'} <span onClick={loginToggleHandler} className="text-blue">{login ? 'Create An Account' : 'login'}</span></p>
            </div>
        </div>
    );
}
