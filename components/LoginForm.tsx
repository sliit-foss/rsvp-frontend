...
const signupFormFields = <form className="flex flex-col w-full">
...
const loginFormFields = <form className="flex flex-col w-full">
    <input type="text" placeholder="Email" className="shadow-ds2 mt-4 border-none text-xs font-semibold h-10"></input>
    <input type="text" placeholder="Password" className="shadow-ds2 mt-4 border-none text-xs font-semibold h-10"></input>
...
const LoginForm = ({login}:LoginFormProps): JSX.Element => {
    return(
        <div className="flex flex-col items-center">
            <h2 className="font-inter font-extrabold text-blue font-inter text-3xl mb-2">{login ? 'LOGIN' : 'Sign Up'}</h2>
             <p className="font-inter text-textBlackSecondary font-medium mb-6 text-sm">{login ? 'Login' : 'Sign up'} to access the best things!</p>
                <button className="shadow-ds2 flex flex-row w-full h-10 rounded-lg flex items-center justify-center leading-4 font-semibold font-inter text-sm mb-6">
                    <NextImage
            ...
            <p className="text-sm border-b-2 w-full text-center mb-2" style={{color:'#D2D2D2',lineHeight:"0.1rem", borderColor:'#D2D2D2'}}><span className="px-3 bg-white">{login ? 'or Sign in with Email' : 'or Signup with Email'}</span></p>
            {login ? loginFormFields : signupFormFields}
            <button className="w-full bg-lightBlue rounded-lg font-inter text-sm leading-6 text-white font-semibold h-10 mt-4 mb-2">{login ? 'Login' : 'Sign Up'}</button>
            <div className="w-full">
                <p className="text-xs text-left font-semibold">{login ? 'Not Registered Yet?': 'Already have an account?'} <span className="text-blue">{login ? 'Create An Account' : 'login'}</span></p>
            </div>
        </div>
    );
}
